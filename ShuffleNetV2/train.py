...
import time
import logging
from network import ShuffleNetV2, ShuffleNetV2M
from utils import accuracy, AvgrageMeter, CrossEntropyLabelSmooth, \
    save_checkpoint, get_lastest_model, get_parameters
...
class OpencvResize(object):
    ...
    def __call__(self, img):
        ...
        H, W, _ = img.shape
        target_size = (int(self.size / H * W + 0.5), self.size) if \
            H < W else (self.size, int(self.size / W * H + 0.5))
...
def get_args():
    ...
    parser.add_argument('--eval-resume', type=str,
                        default='./snet_detnas.pkl',
                        help='path for eval model')
    ...
    parser.add_argument('--auto-continue', type=bool,
                        default=True, help='auto continue')
    parser.add_argument('--display-interval', type=int,
                        default=20, help='display interval')
    parser.add_argument('--val-interval', type=int,
                        default=10000, help='val interval')
    parser.add_argument('--save-interval', type=int,
                        default=10000, help='save interval')

    parser.add_argument('--model-size', type=str, default='1.0x',
...
def main():
    ...
    fh = logging.FileHandler(os.path.join(
        'log/train-{}{:02}{}'.format(local_time.tm_year % 2000,
                                     local_time.tm_mon, t)))
    ...
    model = ShuffleNetV2M(model_size=args.model_size)
    print(model)
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer, lambda step: (1.0 - step / args.total_iters) if
        step <= args.total_iters else 0, last_epoch=-1)
    ...
    while all_iters < args.total_iters:
        ...
        validate(model, device, args, all_iters=all_iters)
    all_iters = train(model, device, args, val_interval=int(
        1280000 / args.batch_size), bn_process=True, all_iters=all_iters)
    validate(model, device, args, all_iters=all_iters)
...
def train(model, device, args, *, val_interval, bn_process=False,
          all_iters=None):
        ...
        if all_iters % args.display_interval == 0:
            printInfo = \
                'TRAIN Iter {}: lr = {:.6f},\tloss = {:.6f},\t'.format(
                    all_iters, scheduler.get_lr()[0], loss.item()) + \
                'Top-1 err = {:.6f},\t'.format(
                    Top1_err / args.display_interval) + \
                'Top-5 err = {:.6f},\t'.format(
                    Top5_err / args.display_interval) + \
