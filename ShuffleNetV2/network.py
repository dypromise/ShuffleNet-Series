...
class ShuffleNetV2M(nn.Module):
    def __init__(self, input_size=224, n_class=1000, model_size='1.0x'):
        ...
        for idxstage in range(len(self.stage_repeats)):
            ...
            seq = [ShuffleV2Block(
                input_channel, output_channel,
                mid_channels=output_channel // 2, ksize=5, stride=2)]

            for i in range(numrepeat - 1):
                seq.append(ShuffleV2Block(
                    output_channel // 2, output_channel,
                    mid_channels=output_channel // 2, ksize=5, stride=1))
    ...
    def forward(self, x):
        ...
        if self.model_size == '2.0x':
            x = self.dropout(x)
        x = x.contiguous().view(-1, self.stage_out_channels[-2])
        x = self.classifier(x)
        return x
